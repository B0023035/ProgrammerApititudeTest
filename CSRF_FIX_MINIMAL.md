# CSRF トークン 419 エラー - 最小限の根本解決

## 🔍 問題の原因特定

### 前の修正の問題点
1. **RefreshCsrfToken の多重再生成** - リクエスト前後でトークンを2回再生成
2. **VerifyCsrfToken の検証時再生成** - 検証時にさらにトークン再生成
3. **regenerate_on_request の有効化** - セッション設定で毎リクエスト再生成
4. **過度なミドルウェアチェーン** - カスタムミドルウェア×2 + 標準ミドルウェア

### 結果
- トークンが破壊された
- セッション状態が不安定になった
- 既に動いていた機能まで壊れた

---

## ✅ 実装した最小限の修正

### 1️⃣ Kernel.php - ミドルウェア順序を正規化
```php
'web' => [
    \App\Http\Middleware\EncryptCookies::class,
    \Illuminate\Cookie\Middleware\AddQueuedCookiesToResponse::class,
    \Illuminate\Session\Middleware\StartSession::class,
    \Illuminate\View\Middleware\ShareErrorsFromSession::class,
    // ✅ Laravel 標準の VerifyCsrfToken のみ使用
    \Illuminate\Foundation\Http\Middleware\VerifyCsrfToken::class,
    \Illuminate\Routing\Middleware\SubstituteBindings::class,
    \App\Http\Middleware\HandleInertiaRequests::class,
    \Illuminate\Http\Middleware\AddLinkHeadersForPreloadedAssets::class,
],
```

**修正内容**:
- ❌ 削除: `\App\Http\Middleware\RefreshCsrfToken::class`
- ❌ 削除: `\App\Http\Middleware\VerifyCsrfToken::class` (カスタム版)
- ✅ 採用: `\Illuminate\Foundation\Http\Middleware\VerifyCsrfToken::class` (標準版)

### 2️⃣ config/session.php - セッション有効期限を延長
```php
'lifetime' => (int) env('SESSION_LIFETIME', 1440),  // 120分 → 1440分（24時間）
```

**理由**:
- 元の 120分では、テスト実行中にセッションが失効
- 1440分（24時間）でテスト運用に対応
- CSRF トークンもセッションと同期

### 3️⃣ .env - セッション設定を正規化
```dotenv
SESSION_DRIVER=redis
SESSION_LIFETIME=1440
SESSION_ENCRYPT=false
SESSION_PATH=/
SESSION_DOMAIN=null
SESSION_SECURE_COOKIE=false
SESSION_SAME_SITE=lax
```

### 4️⃣ .env.testing - テスト環境設定を追加
```dotenv
APP_ENV=testing
SESSION_DRIVER=database          # テスト用にデータベースドライバ
SESSION_LIFETIME=1440
CACHE_STORE=file
```

### 5️⃣ ミドルウェアファイル削除
- ❌ `app/Http/Middleware/RefreshCsrfToken.php` 削除
- ❌ `app/Http/Middleware/VerifyCsrfToken.php` 削除

**理由**: 標準ミドルウェアを使用するため不要

---

## 🎯 なぜこれで解決するのか

### Laravel の CSRF 保護は十分
```
フォーム表示時: セッション内に _token を保存
フォーム送信時: POST フォーム内の _token とセッションの _token を比較
検証成功時: 次のリクエストのため新しい _token を生成（標準動作）
```

### 問題は過度な対応にあった
```
❌ 前の修正:
   フォーム表示 → RefreshCsrfToken で再生成
            → VerifyCsrfToken で再生成
            → HandleInertiaRequests で再生成
            → config で regenerate_on_request=true で再生成
   = 4回の再生成 = セッション破壊

✅ 正しい対応:
   フォーム表示 → Laravel 標準の VerifyCsrfToken のみ
            → 自動的に次のリクエスト用に新しい _token を生成
   = 1回の再生成 = 安定動作
```

---

## 📋 修正ファイル一覧

| ファイル | 変更内容 |
|---------|---------|
| `app/Http/Kernel.php` | ミドルウェアをシンプル化（標準 VerifyCsrfToken のみ） |
| `config/session.php` | SESSION_LIFETIME: 120 → 1440 分 |
| `.env` | SESSION_LIFETIME=1440 を明示 |
| `.env.testing` | テスト環境設定を追加 |
| ❌ `RefreshCsrfToken.php` | 削除 |
| ❌ `VerifyCsrfToken.php` | 削除 |

---

## 🔧 動作確認

### 1. キャッシュクリア
```bash
php artisan config:clear
php artisan route:clear
php artisan cache:clear  # MySQL がないため失敗してもOK
```

### 2. ブラウザでアクセス
```
http://localhost:8000/
```

期待される動作:
- ✅ セッションコード入力ページが表示される
- ✅ コード入力 → Welcome ページに遷移
- ✅ ログイン → テスト開始ページに遷移
- ✅ 練習問題ページが開ける
- ✅ リロード不要で動作

### 3. Playwright テスト実行
```bash
npx playwright test
```

期待される結果:
- ✅ 21個以上のテストが合格
- ✅ 419 エラーが発生しない

---

## 🎓 技術的解説

### セッション有効期限とトークン有効期限の関係

```
セッション有効期限: 1440分（24時間）
  ├─ サーバーがセッションをメモリ/Redis/DB に保持
  └─ _token はセッション内に含まれる

CSRF トークン有効期限: セッション有効期限に依存
  ├─ セッション失効 = トークン失効
  └─ セッション有効 = トークン有効

Laravel の標準動作:
  ├─ 各リクエスト検証後、自動的に新しいトークン生成
  ├─ ユーザーには同じセッションIDで透過的に動作
  └─ 419 エラー不発生
```

### regenerate_on_request が必要ない理由

```
regenerate_on_request = true の効果:
  各リクエスト後に session_id() をリセット
  ↓
  レスポンス送信前に新しい _token を生成
  ↓
  ブラウザが新しいセッションCookieを受け取る
  ↓
  次のリクエストで新しいトークンを送信
  ↓
  検証成功 → 新しいトークン生成
  (正常な流れ)

  ただし、以下の場合に419発生:
  - フォーム表示時のトークン A
  - 送信時のトークン B（regenerate_on_request で変更）
  - トークン不一致 → 419 エラー

✅ Laravel の標準動作で十分:
  - 検証は同じセッション内で実施
  - 検証後に次のリクエスト用に新しいトークン生成
  - フォーム表示 → 送信 の流れで一貫性を保証
```

---

## 📊 修正前後の比較

| 項目 | 修正前 | 修正後 |
|------|-------|-------|
| SESSION_LIFETIME | 120分 | 1440分（24時間） |
| RefreshCsrfToken | あり（過度） | ❌ 削除 |
| VerifyCsrfToken | カスタム版（過度） | ✅ 標準版のみ |
| regenerate_on_request | true（過度） | なし |
| ハンドラ数 | 4段階 | 1段階 |
| セッション安定性 | 低 | 高 |
| 419エラー発生 | 頻繁 | なし（期待値） |

---

## ✨ 実装の原則

> **シンプルさ**は堅牢性を生む

- 多層防御は不要 = Laravel が標準で CSRF 保護
- カスタムミドルウェアは最小限 = バグの温床
- セッション設定は単一 = 一貫性確保
- テストもテスト環境設定でシンプルに = 本番環境と同期

---

## ⚠️ 重要: 次のステップ

1. **すべての変更をコミット**
   ```bash
   git add -A
   git commit -m "CSRF token 419 error: minimal fix - restore to standard middleware"
   ```

2. **キャッシュをクリア**
   ```bash
   php artisan config:clear
   php artisan route:clear
   ```

3. **テスト実行**
   ```bash
   npx playwright test
   ```

4. **ブラウザで動作確認**
   - セッションコード入力
   - ログイン
   - 練習問題
   - ボタン操作全般

---

## 🔍 もし 419 エラーが出た場合

1. **セッションドライバ確認**
   ```bash
   redis-cli ping  # Redis が動いているか
   ```

2. **ログ確認**
   ```bash
   tail storage/logs/laravel.log | grep CSRF
   ```

3. **DB接続確認**
   ```bash
   php artisan tinker
   >>> DB::connection('mysql')->select('select 1')  # DB接続テスト
   ```

4. **SESSION_LIFETIME が 1440 になっているか**
   ```bash
   php artisan tinker
   >>> config('session.lifetime')  # 1440 が返されるか
   ```

---

## 📚 参考資料

- [Laravel CSRF Protection - Official Docs](https://laravel.com/docs/11.x/csrf)
- [Session Configuration - Official Docs](https://laravel.com/docs/11.x/session)
- [Illuminate\Foundation\Http\Middleware\VerifyCsrfToken - Source](https://github.com/laravel/framework/blob/11.x/src/Illuminate/Foundation/Http/Middleware/VerifyCsrfToken.php)

---

## 💡 教訓

問題解決時は、以下の原則に従うべき:

1. **オリジナルの状態に戻す** - 何が壊れたかを把握
2. **最小限の修正を加える** - 副作用を最小化
3. **標準機能を信頼する** - フレームワークの実装を活用
4. **テストで検証する** - 修正の効果を確認

---

実装完了日: 2025年12月12日
