    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                ...
                <a href="{{ route('results.grade-list') }}" class="px-6 py-4 text-sm font-medium text-gray-600 hover:text-blue-600 hover:border-gray-300 border-b-2 border-transparent">
                    学年別一覧
                </a>
                <a href="{{ route('results.statistics') }}" class="px-6 py-4 text-sm font-medium text-blue-600 border-b-2 border-blue-600">
                    統計・グラフ
                </a>
            </nav>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div class="text-center">
                <div class="text-lg font-semibold text-gray-800 mb-2">Part {{ $part }}</div>
                <div class="text-4xl font-bold text-blue-600 mb-2">{{ $average }}%</div>
                <div class="w-full bg-gray-200 rounded-full h-4">
                    <div class="bg-blue-600 h-4 rounded-full transition-all duration-300" style="width: {{ $average }}%"></div>
                </div>
                <div class="text-sm text-gray-600 mt-2">
                    @if($part == 1) 言語能力 (40問)
                    @elseif($part == 2) 図形認識 (30問)
                    @else 計算能力 (25問)
                    @endif
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100">
                            <td class="py-3 text-sm font-medium text-gray-800">
                                Part {{ $part }}
                                @if($part == 1) <span class="text-xs text-gray-500">(言語能力)</span>
                                @elseif($part == 2) <span class="text-xs text-gray-500">(図形認識)</span>
                                @else <span class="text-xs text-gray-500">(計算能力)</span>
                                @endif
                            </td>
                            <td class="py-3 text-right">
                                <span class="text-lg font-bold text-blue-600">{{ $average }}%</span>
                            </td>
                ...
                @foreach($ranges as $range)
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-sm font-bold text-gray-800">{{ $count }}人 ({{ $percentage }}%)</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="bg-{{ $range['color'] }}-500 h-3 rounded-full" style="width: {{ $percentage }}%"></div>
                        </div>
                    </div>
                @endforeach
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // パート別グラフ
    const partAverages = @json($partAverages);
